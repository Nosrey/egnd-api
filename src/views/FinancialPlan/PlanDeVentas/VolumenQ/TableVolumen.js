...
function TableVolumen(props) {
    ...
    const replaceMonth = (producto, indexYear, mes, value) => {
        ...
        const volTotal = Object.values(newMeses).reduce((acc, curr) => acc + parseInt(curr, 10), 0);
        newAños[indexYear] = { ...newAños[indexYear], volMeses: newMeses, volTotal };
    ...
    const handleOnChangeInitialValue = (pais, canalName, prod, newValue, key, mes, indexYear) => {
        ...
        switch (key) {
            ...
            case 'mes':
                producto.años = replaceMonth(producto, indexYear, mes, newValue === '' ? 0 : (newValue[0] === "0" ? newValue.substring(1) : newValue))
                break
            default:
                break
        }
    ...
    const postVolumenData = (data) => {
        createVolumen(data)
            .then(() => {
                ...
                setTimeout(() => {props.showAlertSuces(false)}, 5000)
            })
            .catch((error) => {
                window.scrollTo({ top: 0, behavior: 'smooth' })
                props.showAlertError(true)
                setTimeout(() => {props.showAlertError(false)}, 5000)
            })
    }
                                                                ...
                                                                <FormItem className=" mb-0 w-[130px] ">
                                                                    ...
                                                                    >
                                                                        <Input
                                                                            ...
                                                                            onChange={(e) => handleOnChangeInitialValue(
                                                                                    pais,
                                                                                    canal.canalName,
                                                                                    producto,
                                                                                    e.target.value,
                                                            ...
                                                            <FormItem className=" mb-0 w-[230px] mt-[12px]">
                                                                ...
                                                                >
                                                                    <Select
                                                                        ...
                                                                        value={optionsMonths.filter((option) => option.value === producto.inicioMes)}
                                                                        onChange={(e) => handleOnChangeInitialValue(
                                                                                pais,
                                                                                canal.canalName,
                                                                                producto,
...

            {
                ...
                <div className='bg-indigo-50 px-[25px] py-[30px] pb-[40px] w-fit rounded mt-[60px]'>
                    <div className='flex items-center'>
                        <p className=' text-[#707470] font-bold mb-3 text-left w-[500px] '>Volumen por producto</p>                                   
                    </div>                                             
                    <div className='w-fit pt-3 border border-neutral-600 border-x-0 border-b-0'>
                        {infoProducts.length >0 && infoProducts.map((prod, index ) => (
                                <div key={index} className='flex gap-x-3 w-fit pt-3 '>
                                    <p className={`w-[500px]  pl-[45px] capitalize self-center ${index === 0 ? 'mt-[62px]' : ''}`}>{prod.name}</p>
                                    ...
                                    {prod.sum?.map((año,indexYear) => (
                                        <div className="flex flex-col" key={indexYear}>
                                            ...
                                            <div className="flex gap-x-3 gap-y-3">
                                                {visibleItems.includes(indexYear) && año &&
                                                año.numeros?.map((valor, index ) => (<p   className="w-[90px] text-center">{valor}</p>))  
                                                }
